...
import { applyJobSchema, ApplyJobSchemaType } from "./apply-job-schema";
import { applyForJobAction } from "../actions/apply-job-action";
...
export const ApplyJobModal = ({
  ...
  const onSubmit = async (data: ApplyJobSchemaType) => {
    try {
        console.log(data);
  ...
  if (hasApplied) {
    return (
      <Button
        disabled
        className="w-full sm:w-auto bg-green-300 text-gray-500 cursor-not-allowed"
      >
        Already Applied
      </Button>
    );
  }
    //   <CheckApplied hasApplied={hasApplied} />
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild >
        <Button className="w-full sm:w-auto gap-2 cursor-pointer" variant={"destructive"}>
