...
import { Briefcase, Clock, IndianRupee, MapPin, ArrowLeft, Calendar, Share2, Building2 } from "lucide-react";
import { redirect } from "next/navigation";
import { getCurrentUser } from "@/app/(auth)/_actions/auth.queries";
import { checkIfApplied, getJobIDDetails } from "@/app/(applicant)/_components/find-job-components";
import { prisma } from "@/lib/prisma";
import SaveJobButton from "@/components/general/savedbutton";
import ApplyJob from "@/app/(applicant)/_components/ApplyButton";
      ...
      <div className="sticky top-0 z-40 w-full  backdrop-blur-md border-b border-slate-200 dark:border-slate-800">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <Link href="/dashboard/find-jobs">
            <Button variant="ghost" className="gap-2 text-slate-600 dark:text-slate-400 hover:text-primary transition-all">
              <ArrowLeft size={18} />
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-3 text-primary font-medium">
                <Building2 size={20} />
                <span className="tracking-wide uppercase text-xs">{"Verified Employer"}</span>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                <Badge variant="secondary" className="bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 border-none px-3 py-1">
                  {job.jobType}
                </Badge>
                <Badge variant="outline" className="border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 px-3 py-1">
                  {job.workType}
                </Badge>
            ...
            {job.tags && (
              <Card className="border-slate-200 dark:border-slate-800  shadow-sm">
                <CardContent className="p-6 sm:p-10">
                  <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-6">Skills & Tech Stack</h2>
                  <div className="flex flex-wrap gap-3">
                    {job.tags.split(",").map((tag: string) => (
                      <div key={tag} className=" text-slate-700 dark:text-slate-300 px-4 py-2 rounded-xl text-sm font-medium border border-transparent hover:border-primary/30 transition-all">
                        {tag.trim()}
                      </div>
                    ))}
                  </div>
          ...
          <aside className="lg:col-span-4 space-y-6">
            <Card className="border-slate-200 dark:border-slate-800  sticky top-24 shadow-lg overflow-hidden">
              ...
              <CardContent className="p-6 space-y-8">
                ...
                <div className="space-y-6">
                  <h3 className="font-bold text-slate-900 dark:text-slate-100 uppercase text-xs tracking-widest">Job Overview</h3>
                  ...
                  <div className="space-y-5">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <p className="text-xs text-slate-500 font-medium">Salary Range</p>
                        <p className="text-sm font-bold text-slate-900 dark:text-slate-200">
                          {job.minSalary?.toLocaleString()} - {job.maxSalary?.toLocaleString()}
                          <span className="text-[10px] ml-1 opacity-60">/ {job.salaryPeriod}</span>
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-bold text-slate-900 dark:text-slate-200">{job.location}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-bold text-slate-900 dark:text-slate-200">{job.jobType}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-6 border-t dark:border-slate-800">
                  <ApplyJob jobId={id} isApplied={isApplied} />
                 ...
                 <div className="flex justify-between text-xs text-slate-500">
                    <span className="flex items-center gap-1"><Calendar size={12}/> Posted</span>
